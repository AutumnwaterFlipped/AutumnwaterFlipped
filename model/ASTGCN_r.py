...
class Temporal_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        #  x   torch.Size([32, 157, 1, 4])
        lhs = torch.matmul(torch.matmul(x.permute(0, 3, 2, 1), self.U1), self.U2)  # x.permute(0, 3, 2, 1) torch.Size([32, 4, 1, 157])
        ...
        product = torch.matmul(lhs, rhs)  # (B,T,N)(B,N,T)->(B,T,T) 32*4*4
        E = torch.matmul(self.Ve, torch.sigmoid(product + self.be))  # (B, T, T) *157
